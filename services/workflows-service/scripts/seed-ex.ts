...
const vaultpayWalletKycWorkflow = {
  id: 'vaultpay_wallet_kyc_workflow',
  name: 'vaultpay_wallet_kyc_workflow',
  version: 1,
  definitionType: 'statechart-json',
  definition: {
    id: 'vaultpay_wallet_kyc_workflow',
...
const generateParentVaultpayKycSession = async (prismaClient: PrismaClient) => {
  return await prismaClient.workflowDefinition.create({
    data: vaultpayWalletKycWorkflow,
  });
};
...
async function seed(bcryptSalt: number | string) {
  ...
  const customer = await createCustomer(
    ...
    '1',
    'secret',
    'https://raw.githubusercontent.com/vault-pay/logo/main/vaultpay-new-large-logo.png',
  );
  ...
  // try combining this with kycEmailSessionDefinition
  await createFilter(
    ...
    'individuals',
    {
      ...
      where: {
        workflowDefinitionId: 'vaultpay_wallet_kyc_workflow',
        endUserId: { not: null },
      },
    },
    project1.id,
  ...
  await generateParentVaultpayKycSession(client);
